...
class CatVSDogDataset(utils.Dataset):
    ...
    def load_cat_dog(self, dataset_dir, subset):
        ...
        annotations = [a for a in annotations if a['regions']]    # annotation是每一个字典的值，每一个a都包含了一张图片的所有标注信息
        ...
        for a in annotations:
            ...
            #######################################################################
            # if type(a['regions']) is dict:    # a['regions']的值是字典时的处理情况
            #     polygons = [r['shape_attributes'] for r in a['regions'].values()]
            # else:
            #     polygons = [r['shape_attributes'] for r in a['regions']] 
            #######################################################################

            rects = [r['shape_attributes'] for r in a['regions']]     # a['regions']是一个列表
...
def train(model):
    ...
    # dataset_train.load_balloon(args.dataset, "train")    # 【此处还进行了修改】
    dataset_train.load_cat_dog(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = CatVSDogDataset()
    # dataset_val.load_balloon(args.dataset, "val")    # 【此处还进行了修改】
...
if __name__ == '__main__':
    ...
    # Configurations
    if args.command == "train":
        config = CatVSDogConfig()    # 【此处还进行了修改】
    else:
        class InferenceConfig(CatVSDogConfig):    # 【此处还进行了修改】
